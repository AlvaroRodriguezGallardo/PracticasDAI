...
@login_required   #Lo pongo, aunque no se va a ver el botón a menos que esté iniciada la sesión (creo que es buena práctica en el diseño de la interfaz)
...
def insertarProducto(request):  # Accesible por petición POST de formulario
    ...
    if request.method == 'POST' and request.FILES['imagen']:
        ...
        if form.is_valid():
            ...
            exito = introducirProducto(nombre,precio,descripcion,categoria,imagen_ruta)
            if exito==1:
